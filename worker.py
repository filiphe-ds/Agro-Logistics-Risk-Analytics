...
# Desativa avisos de segurança do site do Porto (necessário para o robô não travar)
...
def coletar_clima():
    ...
    print(f"🛰️ [CLIMA] Iniciando coleta para {len(pontos_monitoramento)} pontos...")
    ...
    for ponto in pontos_monitoramento:
        ...
        try:
            ...
            print(f"❌ Erro no ponto {ponto['loc_id']}: {e}")

    if lista_dfs:
        df_final = pd.concat(lista_dfs, ignore_index=True)
        job = client.load_table_from_dataframe(df_final, TABLE_ID_CLIMA, job_config=job_config)
        job.result()
        print(f"🚀 [CLIMA] {len(df_final)} linhas enviadas ao BigQuery.")

# --- FUNÇÃO 2: LINE-UP (EXTRAÇÃO) ---
def extrair_lineup():
    ...
    return pd.read_html(io.StringIO(str(tabelas[0])))[0]

# --- FUNÇÃO 3: LINE-UP (LIMPEZA E ENVIO) ---
def processar_e_subir_lineup(df_bruto):
    if df_bruto is None:
        print("⚠️ [LINE-UP] Nenhum dado bruto para processar.")
        return

    df_clean = df_bruto.copy()
    
    # 1. Achatamento de Colunas (MultiIndex)
    if isinstance(df_clean.columns, pd.MultiIndex):
        df_clean.columns = [' '.join(col).strip() for col in df_clean.columns.values]
    
    # 2. Mapeamento para Identificação
    mapeamento = {
    ...
    df_clean['quantidade_estimada'] = pd.to_numeric(
        df_clean['quantidade_estimada'].astype(str).str.replace('.', '', regex=False).str.replace(',', '.', regex=False), 
        errors='coerce'
    ).fillna(0)
    ...
    # --- O PULO DO GATO SÊNIOR: FILTRAGEM EXATA ---
    # Aqui listamos APENAS as colunas que existem no seu BigQuery
    colunas_permitidas = [
    ...
    # Filtramos o DataFrame (isso remove o 'nome_navio' que causou o erro)
    ...
    try:
        ...
        job = client.load_table_from_file(io.StringIO(json_data), TABLE_ID_LINEUP, job_config=job_config)
        job.result()
        print(f"✅ [LINE-UP] {len(df_upload)} navios atualizados com sucesso!")
    except Exception as e:
        print(f"❌ Erro Line-up: {e}")

# --- EXECUÇÃO PRINCIPAL (O ROBÔ EM SI) ---
