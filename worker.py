        ...
        for fonte in fontes:
            ...
            for tab in tabelas_html:
                ...
                for _, row in df_temp.iterrows():
                    # --- Lógica de Mapeamento Flexível (CORRIGIDA) ---
                    # Adicionamos 'default=None' para aceitar o segundo argumento
                    ...
                    # Agora as chamadas abaixo vão funcionar perfeitamente
                    imo_val = str(buscar(['imo', 'nº', 'identificacao'], '')).split('.')[0].strip()
                    ...
                    reg = {
                        ...
                        'terminal': str(buscar(['terminal', 'local', 'cais'], 'Area de Fundeio')).strip(),
                        'commodity': buscar(['mercadoria', 'produto', 'carga', 'commodity']),
                        'quantidade_estimada': buscar(['peso', 'ton', 'quantidade'], 0), # Aqui estava o erro!
